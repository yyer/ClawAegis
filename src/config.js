...
export const BLOCK_REASON_PROTECTED_PATH = "安全限制：禁止访问、查询、修改、删除、关闭或绕过受保护的敏感路径、配置、重要 skill 或 clawaegisex 插件目录。";
...
export function userConfigCandidatePaths(rootDir) {
    const out = [];
    const home = os.homedir();
    if (rootDir)
        out.push(path.join(rootDir, "user_config.json"));
    if (home) {
...
export function getUserConfigMtimeMs(rootDir) {
...
export function findUserConfigPath(rootDir) {
...
export function resolveClawAegisPluginConfig(api) {
    ...
    const outboundTrustedEndpoints = Array.isArray(raw.outboundTrustedEndpoints)
        ? raw.outboundTrustedEndpoints
            ...
            fingerprint: typeof e.fingerprint === "string"
                ? e.fingerprint.toLowerCase()
                : "",
