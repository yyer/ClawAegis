...
function wrapHookFailOpenSync(api, hookName, handler) {
    return (event, ctx) => {
        try {
            ...
            api.logger.error(`[clawaegisex] ${hookName} failed; fail-open keeps OpenClaw running: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    };
}
    ...
    try {
        ...
        const reload = () => {
            try {
                ...
                api.logger.error(`[clawaegisex] hot-reload failed; keeping previous runtime: ${error instanceof Error ? error.message : String(error)}`);
            }
        };
        ...
        const dispatch = (name) => wrapHookFailOpen(api, String(name), (event, ctx) => {
        ...
        api.logger.error(`[clawaegisex] register failed; fail-open keeps OpenClaw running: ${error instanceof Error ? error.message : String(error)}`);
    }
}
